...
public class BigRoadTilePane extends TilePane {
    ...
    private void ReArrangeElements(ListProperty<BeadRoadResult> beadRoadList) {
        getChildren().stream().map(x -> (BigRoadLabel)x ).forEach(t-> ClearLabel(t));
        row = -1;column = 0;savedColumn= -1;
        c0 = c1 = c2 = c3 = c4 = 0;
    ...
    public void AddElement(ListProperty<BeadRoadResult> beadRoadList) {
    ...
    public void RemoveElement(ListProperty<BeadRoadResult> beadRoadList) {
        Long ballsBefore = getCount();
        ReArrangeElements(beadRoadList);
        Long ballsAfter = getCount();
