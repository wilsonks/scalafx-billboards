...
import customjavafx.scene.control.SmallRoadDummyLabel;
...
public class SmallRoadDummyTilePane extends TilePane {

    private int column = 0;
    private int row = -1;
    private int savedColumn = -1;

    public SmallRoadDummyTilePane() {
        super();
    ...
    private void Insert() {
        this.getChildren().add(new SmallRoadDummyLabel(SmallRoadResult.EMPTY));
    }
    ...
    private void ClearLabel(SmallRoadDummyLabel t) {
        t.setResult(SmallRoadResult.EMPTY);
    }

    private boolean isCurrentWinRed() {
        switch (((SmallRoadDummyLabel) super.getChildren().get(getCurrentPosition())).getResult()) {
            case RED:
                return true;
            default:
                return false;
    ...
    private void MoveForSameColor() {
        if ((getPrefRows() - (row + 1)) == 0) {
            ...
            if (((SmallRoadDummyLabel) super.getChildren().get(getCurrentPosition() + 1)).getResult() == SmallRoadResult.EMPTY){
                row++;
            }
            else {
                if(savedColumn == -1) savedColumn = column;
    ...
    private void AddElement(SmallRoadResult next) {
        ...
        ((SmallRoadDummyLabel)getChildren().get(getCurrentPosition())).setResult(next);
    }

    public void ReArrangeElements(ObservableList<SmallRoadDummyLabel> labels) {
        getChildren().stream().map(x -> (SmallRoadDummyLabel)x ).forEach(t-> ClearLabel(t));
    ...
    public void Reset(){
        getChildren().stream().map(t->(SmallRoadDummyLabel)t).forEach(t->{
            t.setResult(SmallRoadResult.EMPTY);
        });
        column = 0;
        row= -1;
