...
import customjavafx.scene.control.CockroachRoadDummyLabel;
...
public class CockroachRoadDummyTilePane extends TilePane {

    private int column = 0;
    private int row = -1;
    private int savedColumn = -1;

    public CockroachRoadDummyTilePane() {
        super();
    ...
    private void Insert() {
        this.getChildren().add(new CockroachRoadDummyLabel(CockroachRoadResult.EMPTY));
    }
    ...
    private void ClearLabel(CockroachRoadDummyLabel t) {
        t.setResult(CockroachRoadResult.EMPTY);
    }

    private boolean isCurrentWinRed() {
        switch (((CockroachRoadDummyLabel) super.getChildren().get(getCurrentPosition())).getResult()) {
            case RED:
                return true;
            default:
                return false;
    ...
    private void MoveForSameColor() {
        if ((getPrefRows() - (row + 1)) == 0) {
            ...
            if (((CockroachRoadDummyLabel) super.getChildren().get(getCurrentPosition() + 1)).getResult() == CockroachRoadResult.EMPTY){
                row++;
            }
            else {
                if(savedColumn == -1) savedColumn = column;
    ...
    private void AddElement(CockroachRoadResult next) {
        ...
        ((CockroachRoadDummyLabel)getChildren().get(getCurrentPosition())).setResult(next);
    }

    public void ReArrangeElements(ObservableList<CockroachRoadDummyLabel> labels) {
        getChildren().stream().map(x -> (CockroachRoadDummyLabel)x ).forEach(t-> ClearLabel(t));
    ...
    public void Reset(){
        getChildren().stream().map(t->(CockroachRoadDummyLabel)t).forEach(t->{
            t.setResult(CockroachRoadResult.EMPTY);
        });
        column = 0;
        row= -1;
